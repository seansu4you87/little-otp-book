defmodule Manager do
  ...
  # It's important to separate out a Client API, to ensure that it's calling
  # into GenServer callbacks, so that important operations are done in context
  # of the GenServer process, not the calling client.
  ...
  def start_children([head|tail]) do
    case start_child(head) do
      {:ok, pid} ->
        [{pid, head}|start_children(tail)]
      :error -> :error
    end
  end
