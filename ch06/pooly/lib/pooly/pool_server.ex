defmodule Pooly.PoolServer do
  ...
  def handle_info(:start_worker_supervisor, state = %{
    pool_sup: pool_sup,
    name: name,
    mfa: mfa,
    size: size
  ...
  def handle_info({:DOWN, ref, _, _, _}, state = %{
    monitors: monitors,
    workers: workers
  }) do
  ...
  def handle_call(:checkout, {consumer, _ref}, %{
    workers: workers,
    ...
    case workers do
      [head|tail] ->
        ...
        true = :ets.insert(monitors, {head, ref})
        {:reply, head, %{state | workers: tail}}
      [] -> {:reply, :noproc, state}
    end
  end
  def handle_call({:checkout, block}, {consumer, _ref} = from, state) do
    %{
    ...
    case workers do
      ...
      [] when max_overflow > 0 and overflow < max_overflow ->
        {worker, ref} = new_worker(worker_sup, consumer)
